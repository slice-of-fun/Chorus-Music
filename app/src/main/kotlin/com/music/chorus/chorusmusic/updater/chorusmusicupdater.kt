...
package pushkar.chorus.music.chorusmusic.updater
...
import pushkar.chorus.music.chorusmusic.updater.downloadmanager.UpdateDownloadWorker
import pushkar.chorus.music.chorusmusic.updater.downloadmanager.DownloadNotificationManager
...
import pushkar.chorus.music.ui.component.ChangelogItem
import pushkar.chorus.music.ui.component.leadingItemShape
import pushkar.chorus.music.ui.component.middleItemShape
import pushkar.chorus.music.ui.component.endItemShape
import pushkar.chorus.music.ui.component.detachedItemShape
import pushkar.chorus.music.ui.component.parseMarkdown
import pushkar.chorus.music.ui.component.endItemShape
import pushkar.chorus.music.ui.component.detachedItemShape
import pushkar.chorus.music.ui.component.AnimatedActionButton
import pushkar.chorus.music.ui.component.ExpressiveIconButton
import pushkar.chorus.music.ui.component.ErrorSnackbar
...
sealed class ChorusUpdateStatus {
    object Idle : ChorusUpdateStatus()
    object Checking : ChorusUpdateStatus()
    ...
    data class NoUpdate(val version: String) : ChorusUpdateStatus()
    data class Error(val message: String) : ChorusUpdateStatus()
}
...
fun UpdateScreen(navController: NavHostController) {
    ...
    var status by remember { mutableStateOf<ChorusUpdateStatus>(ChorusUpdateStatus.NoUpdate(BuildConfig.VERSION_NAME)) }
    ...
    fun triggerUpdateCheck() {
        status = ChorusUpdateStatus.Checking
        scope.launch {
            ...
            checkForUpdate(
                ...
                onSuccess = { tag, isAvailable, changelog, size, date, description, imageUrl, apkUrl ->
                    ...
                    status = if (isAvailable) {
                        ...
                        ChorusUpdateStatus.NoUpdate(tag)
                    }
                },
                onError = {
                    status = ChorusUpdateStatus.Error(context.getString(R.string.cant_check_updates))
                }
            )
        }
    }
    ...
    Scaffold(
        topBar = {
            LargeTopAppBar(
                title = {
                    val titleText = if (status is ChorusUpdateStatus.Available) {
                        buildAnnotatedString {
                            ...
                            withStyle(
                                ...
                                append((status as ChorusUpdateStatus.Available).version)
                            }
                        }
                    } else {
            ...
            Surface(
                ...
                Column(
                    ...
                    when (val currentStatus = status) {
                        is ChorusUpdateStatus.Idle, is ChorusUpdateStatus.Checking, is ChorusUpdateStatus.NoUpdate, is ChorusUpdateStatus.Error -> {
                            AnimatedActionButton(
                                text = stringResource(R.string.check_for_update),
                                onClick = { triggerUpdateCheck() },
                                enabled = currentStatus !is ChorusUpdateStatus.Checking && !isDownloading,
                                modifier = Modifier.fillMaxWidth()
                            )
                        }

                        is ChorusUpdateStatus.Available -> {
                            Row(
                                modifier = Modifier.fillMaxWidth(),
        ...
        Box(
            ...
            LazyColumn(
                ...
                item {
                    val contentModifier = if (status is ChorusUpdateStatus.Available) {
                    ...
                    Box(
                        ...
                        when (val currentStatus = status) {
                            is ChorusUpdateStatus.Checking -> {
                                androidx.compose.material3.ContainedLoadingIndicator(
                                    modifier = Modifier.size(64.dp)
                                )
                            }

                            is ChorusUpdateStatus.NoUpdate -> {
                                Column(
                                    horizontalAlignment = Alignment.CenterHorizontally
                                ) {
                                    Icon(
