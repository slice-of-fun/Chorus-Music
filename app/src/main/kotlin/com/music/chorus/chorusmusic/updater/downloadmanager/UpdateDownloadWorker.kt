package pushkar.chorus.music.chorusmusic.updater.downloadmanager
...
class UpdateDownloadWorker(private val context: Context, workerParams: WorkerParameters) :
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val downloadFile = if (isZip) File(downloadDir, "chorus_temp.zip") else File(downloadDir, "chorusmusic.apk")
            ...
            val finalFile = if (isZip) {
                val targetApkFile = File(downloadDir, "chorusmusic.apk")
                var extracted = false
                try {
